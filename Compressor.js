...
function CompressorComp() {
    ...
    useEffect(() => {
        if (originalSize) {
            // Estimate the compressed size based on the compression quality
            const estimatedSize = Math.round(originalSize * compressionQuality);
            setCompressedSize(estimatedSize);
        }
    }, [compressionQuality, originalSize]);
                ...
                <div>
                    <Navbar.Brand className="navbar-content">
                        ...
                        <div className="namenavbar">
                            <center>Image Compressor</center>
                        </div>
                    </Navbar.Brand>
                </div>
                            ...
                            <div>
                                ...
                                <div className="text-center">
                                    {isCompressed && !compressionInProgress && (
                                        <div className="text-success compressed-message">Image compressed successfully!</div>
                                    )}
                ...
                <div className="col-xl-3 col-lg-3 col-md-12 col-sm-12">
                    {showCompressedImage ? (
                        <div>
                            ...
                            <Modal show={modalShow} onHide={() => setModalShow(false)} size="lg">
                                <Modal.Body className="text-center">
                                    <Card.Img className="image" variant="top" src={compressedLink} alt="Compressed Image" />
                                </Modal.Body>
                                <Modal.Footer>
                        ...
                        <div className="d-flex align-items-center justify-content-center">
                            {compressionInProgress && <Spinner animation="border" variant="primary" />}
                            {!uploadImage && !compressionInProgress && (
                                <FontAwesomeIcon icon={faImage} className="icon" size="3x" />
                            )}
                        </div>
                    )}
                </div>
